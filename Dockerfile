...
# Misc
LABEL Description="DistribD Docker image based on Alpine" Vendor="Alain Lefebvre" 
MAINTAINER Alain Lefebvre <hartfordfive@gmail.com>

ARG VERSION
ENV VERSION=$VERSION
ARG FIRST_NODE
ENV FIRST_NODE=$FIRST_NODE

RUN set -ex ;\
    # Ensure protologbeat user exists
    addgroup -S distribd && adduser -S -G distribd distribd ;\
...
COPY bin/distribd /opt/distribd/distribd


WORKDIR /opt/protologbeat
USER protologbeat

CMD ["distribd", "-haddr", ":11000", "-raddr",  ":12000"]
